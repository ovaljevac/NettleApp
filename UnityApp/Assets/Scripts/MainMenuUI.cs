    ...
    private void CreateInfoUI()
{
    ...
    // ---------------- SLIKA (sa paddingom i zaobljenim uglovima) ----------------
    // Kontejner koji daje padding
    GameObject imgContainerGO = new GameObject("PlantImageContainer");
    imgContainerGO.transform.SetParent(infoPanel.transform, false);
    RectTransform imgRT = imgContainerGO.AddComponent<RectTransform>();

    // zauzima srednji gornji dio ekrana (ispod naslova)
    imgRT.anchorMin = new Vector2(0.08f, 0.57f);
imgRT.anchorMax = new Vector2(0.92f, 0.82f);

    imgRT.offsetMin = new Vector2(0f, 0f);
    imgRT.offsetMax = new Vector2(0f, 0f);

    // stvarna slika
    Image plantImg = imgContainerGO.AddComponent<Image>();
    ...
    // ako koristiš sprite sa zaobljenim uglovima i podešenim borderom,
    // ovo će ga nacrtati kao zaobljeni okvir:
...
bodyRT.anchorMin = new Vector2(0.08f, 0.18f);
bodyRT.anchorMax = new Vector2(0.92f, 0.56f); 
